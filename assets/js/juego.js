...
// Referencia HTML
const nombreJugador = document.querySelector("#name-user");
...
const checkNameUser = (func) => {
  /**
   * capturo el nodo dentro de la etiqueta h1
   * capturo el small de su pocicion inicial
   * modificamos el nombre del usuario
   */
  const nombrePlayer = prompt("Escriba su nombre");
  if (nombrePlayer === "") {
    return func("Debe ingresar un nombre de usuario");
  }

  return (nombreJugador.childNodes[0].innerText = nombrePlayer);
};

checkNameUser((name) => alert(name));
